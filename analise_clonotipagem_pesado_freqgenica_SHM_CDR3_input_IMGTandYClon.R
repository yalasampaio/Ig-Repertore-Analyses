...
meus_arquivos <- "/home/yala/Documentos/YALA/5.CLONOTIPAGEM-ANALISE/PESADA"
...
for(y in 1:nrow(INPUT)){
 print(INPUT[y,])
  ...
  #                             sep="/"),
  #                       header = TRUE, sep = "\t")
  CLONOTIPAGEM <- CLONOTIPAGEM %>% select(sequence_id, v_call, v_identity, d_call, j_call, j_identity, cdr3, junction_aa, clone_id, clone_seq_count)
  ...
  #Manter na coluna apenas o 1o anotação e deletar tudo depois da vírgula e do asterisco
  CLONOTIPAGEM$v_call <- sub(",.*", "", CLONOTIPAGEM$v_call)
  #CLONOTIPAGEM$v_call <- sub("\\*.*", "", CLONOTIPAGEM$v_call)
  #CLONOTIPAGEM$v_call <- sub("\\*.*|-.*", "", CLONOTIPAGEM$v_call)
  CLONOTIPAGEM$v_call <- sub("\\*.*|Equcab", "", CLONOTIPAGEM$v_call)
  
  CLONOTIPAGEM$j_call <- sub(",.*", "", CLONOTIPAGEM$j_call)
  #CLONOTIPAGEM$j_call <- sub("\\*.*", "", CLONOTIPAGEM$j_call)
  #CLONOTIPAGEM$j_call <- sub("\\*.*|-.*", "", CLONOTIPAGEM$j_call)
  CLONOTIPAGEM$j_call <- sub("\\*.*|Equcab", "", CLONOTIPAGEM$j_call)
  
  #CLONOTIPAGEM$d_call <- sub("\\*.*|-.*", "", CLONOTIPAGEM$d_call)
  CLONOTIPAGEM$d_call <- sub("\\*.*|Equcab", "", CLONOTIPAGEM$d_call)
  ...
  #D
  ValorDabsoluto <- table(REPORT$d_call)
  ValorDrelativo <- prop.table(table(REPORT$d_call))
  TabelaGeneD <- data.frame (ValorDabsoluto, ValorDrelativo)
  
  #Hipermutacao somatica
  #Colocar as sequencias de forma rankeada pela coluna do clone_seq_count
  #V
  hiperV <- REPORT %>%
    mutate(v_identity = 1 - v_identity) %>%
    group_by(v_call) %>%
    summarise(hipermutacao_v = mean(v_identity)) 
  #J
  hiperJ <- REPORT %>%
    mutate(j_identity = 1 - j_identity) %>%
    group_by(j_call) %>%
    summarise(hipermutacao_j = mean(j_identity)) 
  
  #Análise do CDR3
  #Tamanho
  REPORT$cdr3_tam <- nchar(REPORT$junction_aa)
  #Cria uma tabela de frequencia dos tamanhos do junction_aa
  ...
  # Cria novo data frame apenas com a coluna junction_aa da REPORT
  Analise_CDR3 <- data.frame(junction_aa = REPORT$junction_aa)
  #cria 20 colunas no data frame, e conta quantos caracteres tem na sequencia de aa na coluna junction_aa
  ...
  for (aa in amino_acidos) {
    Analise_CDR3[[aa]] <- str_count(REPORT$junction_aa, aa)
  }
   COMP_CDR3 <- list()
  ...
  # Uma unica tabela do excel com 6 abas
   ...
   addWorksheet(workbook, sheetName = "Freq_D")
   writeData(workbook, sheet = 2, x = TabelaGeneD)
   addWorksheet(workbook, sheetName = "Freq_J")
   writeData(workbook, sheet = 3, x = TabelaGeneJ)
   addWorksheet(workbook, sheetName = "Hiper_V")
   writeData(workbook, sheet = 4, x = hiperV)
   addWorksheet(workbook, sheetName = "Hiper_J")
   writeData(workbook, sheet = 5, x = hiperJ)
   addWorksheet(workbook, sheetName = "Tamanho_CDR3")
   writeData(workbook, sheet = 6, x = TAM_CDR3)
   addWorksheet(workbook, sheetName = "Composicao_CDR3")
   writeData(workbook, sheet = 7, x = COMP_CDR3)

   saveWorkbook(workbook, paste(meus_arquivos,
                                str_replace(INPUT$report[y],
                                                          "report.tsv.gz",
                                                          "analise_resultado4.xls"),
                                sep = "/"))
